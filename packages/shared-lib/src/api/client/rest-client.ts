...
import { combineInterceptors, loggingInterceptor, retryInterceptor, rateLimitInterceptor } from './interceptors'
import { withRetry, withRateLimitRetry } from './retry'
...
const parseLinkHeader = (linkHeader: string | null): { next?: string; prev?: string; first?: string; last?: string } => {
...
export const makeRestClient = (config: RestClientConfig): ApiClient => {
  ...
  return {
    ...
    get: <T>(path: string, params?: Record<string, unknown>) =>
      request<T>({ method: 'GET', path, params }).pipe(
    ...
    delete: <T>(path: string, params?: Record<string, unknown>) =>
      request<T>({ method: 'DELETE', path, params }).pipe(
        Effect.map((response) => response.data)
      ),
        ...
        Effect.map((response) => {
          ...
          return {
            ...
            pageInfo: {
              ...
              endCursor: links.next ? new URL(links.next).searchParams.get('page') || undefined : undefined,
            },
          }
        })
      ),
